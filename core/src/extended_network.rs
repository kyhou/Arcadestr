...
use crate::nostr::{NostrClient, KIND_FOLLOW_LIST, KIND_RELAY_LIST, parse_relay_list_from_event};
...
pub enum DiscoveryState {
    Idle,
    FetchingFollowLists { fetched: usize, total: usize, coverage_percent: u8 },
...
impl ExtendedNetworkRepository {
    ...
    pub fn clear(&self) -> Result<(), ExtendedNetworkError> {
        self.social_graph.clear_all()
            .map_err(|e| ExtendedNetworkError::Database(e.to_string()))?;
        *self.discovery_state.lock().expect("discovery_state mutex poisoned") = DiscoveryState::Idle;
        *self.cached_network.lock().expect("cached_network mutex poisoned") = None;
        *self.discovery_in_progress.lock().expect("discovery_in_progress mutex poisoned") = false;
        Ok(())
    }
    
    pub fn get_state(&self) -> DiscoveryState {
        self.discovery_state.lock().expect("discovery_state mutex poisoned").clone()
    }
    
    pub fn get_cached_network(&self) -> Option<ExtendedNetworkCache> {
        self.cached_network.lock().expect("cached_network mutex poisoned").clone()
    }
    
    pub fn is_cache_stale(&self) -> bool {
        let cache = self.cached_network.lock().expect("cached_network mutex poisoned");
        match cache.as_ref() {
            None => true,
            Some(c) => {
    ...
    pub async fn discover_network(
        ...
        {
            let mut in_progress = self.discovery_in_progress.lock().expect("discovery_in_progress mutex poisoned");
            if *in_progress {
    ...
    async fn _discover_network(
        ...
        info!("Starting streaming extended network discovery with {} first-degree follows", total_first);
        
        // Initialize state
        *self.discovery_state.lock().expect("discovery_state mutex poisoned") = DiscoveryState::FetchingFollowLists {
            fetched: 0,
            total: total_first,
            coverage_percent: 0,
        };
        ...
        if authors.is_empty() {
            return Err(ExtendedNetworkError::Nostr("No valid pubkeys to query".to_string()));
        }
        ...
        match timeout(
            ...
            Ok(Ok(events)) => {
                ...
                info!("Connected to {} new relays from NIP-65 lists", connected_relays);
            }
        ...
        match timeout(
            ...
            Ok(Ok(events)) => {
                for event in events {
                    ...
                    let mut state = self.discovery_state.lock().expect("discovery_state mutex poisoned");
                    *state = DiscoveryState::FetchingFollowLists {
                    ...
                    if fetched % 10 == 0 || fetched == total_first {
                        info!("Follow list progress: {}/{} ({}%)", fetched, total_first, coverage_percent);
                    }
                }
            }
        ...
        if qualified.is_empty() {
            ...
            *self.discovery_state.lock().expect("discovery_state mutex poisoned") = DiscoveryState::Complete { stats: stats.clone() };
        ...
        *self.cached_network.lock().expect("cached_network mutex poisoned") = Some(cache);
    ...
    async fn build_social_graph(
        ...
        for (follower_pubkey, event) in follow_lists {
            ...
            for tag in event.tags.iter() {
                let tag_vec: Vec<String> = tag.clone().to_vec().iter().map(|s| s.to_string()).collect();
        ...
        *self.discovery_state.lock().expect("discovery_state mutex poisoned") = DiscoveryState::BuildingGraph { processed: total, total };
    ...
    async fn fetch_relay_lists_for_pubkeys(
        ...
        for (i, chunk) in chunks.iter().enumerate() {
            ...
            match timeout(timeout_duration, nostr_client.fetch_from_indexers_then_all(filter)).await {
                Ok(Ok(events)) => {
                    for event in events {
            ...
            *self.discovery_state.lock().expect("discovery_state mutex poisoned") = DiscoveryState::FetchingRelayLists {
    ...
    fn compute_relay_set_cover(
        ...
        debug!(
            ...
            from_relay_lists, from_hints, relay_to_authors.len()
        );
        ...
        debug!("Set-cover: Starting with {} qualified pubkeys to cover", uncovered.len());
        ...
        debug!(
            ...
            covered_count,
            qualified.len(),
            if qualified.len() > 0 { (covered_count * 100) / qualified.len() } else { 0 }
        );
