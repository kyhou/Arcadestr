...
use nostr::{Event, PublicKey, UnsignedEvent, signer::NostrSigner};
use std::sync::Arc;
...
pub async fn nip44_encrypt(
    ...
    info!("nip44_encrypt: Encrypting message for {}", recipient_pubkey.to_hex());
...
pub async fn nip44_decrypt(
    ...
    info!("nip44_decrypt: Decrypting message from {}", sender_pubkey.to_hex());
...
pub async fn get_relay_status(
    ...
    match &state_guard.active_client {
        Some(client) => {
            ...
            let relay_urls: Vec<String> = relays.keys()
                .map(|url| url.to_string())
                .collect();
            info!("get_relay_status: Retrieved status for {} relays", relay_urls.len());
            Some(relay_urls)
        }
        None => {
