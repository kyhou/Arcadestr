use crate::storage::{Database, MasterKeyManager, Encryption, EncryptedData, DatabaseError, MasterKeyError, EncryptionError};
use crate::auth::{Account, SigningMode, AccountInfo};
...
impl AccountManager {
    ...
    pub fn get_signer(&self, account: &Account) -> Result<Box<dyn NostrSigner>, AccountManagerError> {
        match account.signing_mode {
            SigningMode::Local => {
                let encrypted_bytes = account.encrypted_nsec.as_ref()
                    .ok_or(AccountManagerError::Signer(
                        SignerError::NoPrivateKey
                    ))?;
    ...
    pub fn get_nsec(&self, account: &Account) -> Result<Zeroizing<String>, AccountManagerError> {
        match account.signing_mode {
            ...
            _ => Err(AccountManagerError::Signer(
                SignerError::NoPrivateKey
            ))
        }
    }
    ...
    pub async fn list_accounts(&self) -> Result<Vec<AccountInfo>, AccountManagerError> {
        let accounts = sqlx::query_as::<_, Account>(
            "SELECT * FROM accounts ORDER BY last_used DESC"
        )
    ...
    pub async fn update_profile(
        ...
        sqlx::query(
            "UPDATE accounts SET display_name = ?, picture = ? WHERE id = ?"
        )
        .bind(&display_name)
        .bind(&picture)
        .bind(account_id)
    ...
    /// Get account by ID
    pub async fn get_account(&self, account_id: &str) -> Result<Option<Account>, AccountManagerError> {
        let account = sqlx::query_as::<_, Account>(
            "SELECT * FROM accounts WHERE id = ?"
        )
        .bind(account_id)
    ...
    /// Get account by npub
    pub async fn get_account_by_npub(&self, npub: &str) -> Result<Option<Account>, AccountManagerError> {
        let account = sqlx::query_as::<_, Account>(
            "SELECT * FROM accounts WHERE npub = ?"
        )
        .bind(npub)
        .fetch_optional(self.db.pool())
        .await?;
    ...
    pub async fn backup_accounts(&self) -> Result<String, AccountManagerError> {
        ...
        // Create backup
        let backup_data = backup.create_backup(&full_accounts)
            .map_err(|e| AccountManagerError::Encryption(
                crate::storage::EncryptionError::Serialization(e.to_string())
            ))?;
    ...
    pub async fn restore_accounts(&self, backup_data: &str) -> Result<usize, AccountManagerError> {
        use crate::storage::{RelayBackup, BackupAccountData};
        ...
        // Restore backup
        let restored_accounts: Vec<BackupAccountData> = backup.restore_backup(backup_data)
            .map_err(|e| AccountManagerError::Encryption(
                crate::storage::EncryptionError::Serialization(e.to_string())
            ))?;
        ...
        for backup_acc in restored_accounts {
            ...
            let encrypted_nsec = backup_acc.encrypted_nsec
